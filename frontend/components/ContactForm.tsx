...
export function ContactForm() {
  ...
  return (
    <div className="rounded-[2.25rem] border border-white/10 bg-gradient-to-br from-slate-950/85 via-indigo-950/70 to-slate-950/85 p-8 shadow-[0_40px_120px_-50px_rgba(37,99,235,0.35)] lg:p-12">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6">
          ...
          <div>
            ...
            <select
              ...
              defaultValue=""
              className={cn(
                "mt-2 w-full rounded-2xl border border-white/10 bg-slate-950/65 px-4 py-3 text-sm text-white outline-none transition focus:border-indigo-400/70 focus:ring-2 focus:ring-indigo-500/40"
              )}
          ...
          <div>
            ...
            <textarea
              ...
              className="mt-2 h-36 w-full rounded-2xl border border-white/10 bg-slate-950/65 px-4 py-3 text-sm text-white outline-none transition focus:border-indigo-400/70 focus:ring-2 focus:ring-indigo-500/40"
          ...
          <div aria-live="polite" role="status" className="text-sm">
            {formState.status === "success" && (
              <p className="rounded-full border border-emerald-400/40 bg-emerald-500/10 px-4 py-2 text-emerald-200">
                Thanks! We received your request. Expect a reply from sagar@callsphere.tech within
                one business day.
              </p>
            )}
            {formState.status === "idle" && formState.error && (
              <p className="rounded-full border border-red-400/40 bg-red-500/10 px-4 py-2 text-red-200">
                {formState.error}
              </p>
            )}
          </div>
        </form>
        <aside className="space-y-6 rounded-3xl border border-white/10 bg-slate-950/70 p-6 shadow-inner shadow-indigo-900/20">
          ...
          <div className="space-y-4 text-sm text-muted-foreground">
            <div>
              <span className="block text-xs uppercase tracking-wide text-indigo-200">
                Address
              </span>
